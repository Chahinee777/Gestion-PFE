...
if(isset($_POST['login'])) 
{
    ...
    $sql = "SELECT ID, NomUtilisateur, MotDePasse FROM tblenseignant WHERE (NomUtilisateur=:username OR NumeroContact=:username) AND MotDePasse=:password";
    ...
    if($query->rowCount() > 0) {
        foreach ($results as $result) {
            $_SESSION['sturecmsstuid'] = $result->NomUtilisateur;
            $_SESSION['sturecmsuid'] = $result->ID;
            // Assuming you have a field named 'ClasseDeLEnseignant' in tblenseignant
            $_SESSION['stuclass'] = $result->ClasseEnseignant;
        }
...
<html lang="en">
  <head>
    ...
    <title>Gestion (PFE) ISET Radès || Page de connexion de l'enseignant</title>
    <!-- plugins:css -->
